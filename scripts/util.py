...
def save_hypotheses(model_name: str, model_size: str, dataset_name: str, sample_rate: int, stm_ids: list, utterance_dict: dict, transcript_dict: dict, start_line: int, end_line: int, hypothesis_file_name: str, job_number: int, total_jobs: int):
    ...
    #==================================================================
    import whisper
    model = whisper.load_model(model_size) # available models = ['tiny.en', 'tiny', 'base.en', 'base', 'small.en', 'small', 'medium.en', 'medium', 'large-v1', 'large-v2', 'large-v3', 'large', 'large-v3-turbo', 'turbo']
    logging.info(
    ...
    with open(hypothesis_file_name, 'w') as output_file:
        for stm_id in stm_ids[start_line:end_line]:
            ...
            print(f'{transcript_dict[stm_id]["stm_info"]} {normalization(hypothesis)}', file=output_file)

#==================================================================
# compute WER
#==================================================================
def save_labels(stm_ids, utterance_dict, reference_dict, hypothesis_dict, start_line, end_line, label_file_full_path, compute_alignments=True, scoring_mode='all', duration_limit=float('inf')):
    try:
    ...
    with open(label_file_full_path, 'w') as output_file:
        ...
        for detail in wer_details:
            #logging.info(detail)
            if detail['hyp_absent'] and detail['key'] not in stm_ids[start_line:end_line]:
                continue
            stm_id = detail['key']
